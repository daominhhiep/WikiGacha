...
export function MainLayout() {
  ...
  const isFixedLayoutPage = ['/battle', '/gacha'].includes(location.pathname);

  return (
    <div
      className={cn(
        'relative flex flex-col bg-background text-foreground selection:bg-primary selection:text-black',
        isFixedLayoutPage ? 'h-screen overflow-hidden' : 'min-h-screen',
      )}
    >
      {/* HUD Noise Overlay */}
      <div className="fixed inset-0 pointer-events-none z-[100] opacity-[0.03] bg-[url('https://grainy-gradients.vercel.app/noise.svg')]" />
      <Navbar />
      <main className={cn('flex-1 relative', isFixedLayoutPage ? 'overflow-hidden' : '')}>
        <div className={cn('container mx-auto px-4', isFixedLayoutPage ? 'h-full py-4' : 'py-8')}>
          <Outlet />
        </div>
      </main>
